...
// ── FIX: explicit glow shadow per color — visible in both light and dark ──
// Previously 'secondary' used var(--shadow-neon-subtle) which was too faint
// in light mode. These inline shadows are always strong enough to see.
...
// ── Border per color — replaces the generic border-white/5 on GlassCard ──
...
function ProficiencyBar({ proficiency, color, animate }: {
  proficiency: number; color: 'primary' | 'secondary' | 'accent'; animate: boolean;
}) {
  const barColor =
    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-1.5">
        ...
        <motion.span
          ...
          initial={{ opacity: 0 }} animate={{ opacity: animate ? 1 : 0 }}
...
function SkillRow({ skill, isActive, onClick, index, inView }: {
  skill: Skill; isActive: boolean; onClick: () => void; index: number; inView: boolean;
}) {
  const barColor =
          ...
          <div className="flex flex-col gap-1">
            {familiarSkills.map((skill, i) => (
              <SkillRow
                ...
                isActive={activeSkillId === skill.id}
                onClick={() => setActiveSkillId(skill.id)}
                index={coreSkills.length + i} inView={inView}
              />
            ))}
          </div>
        </div>
        ...
        <div className="w-full lg:w-[45%]">
          <div className="lg:sticky lg:top-28">
            <AnimatePresence mode="wait">
              ...
              >
                {/*
                  FIX: Don't rely on GlassCard's glowColor prop for the shadow —
                  use an explicit inline boxShadow + borderColor that's always
                  strong enough in both light and dark mode.
                */}
                <div
