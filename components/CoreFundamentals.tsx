...
/**
 * Generate log timestamps relative to now.
 * offsets = seconds ago for each line (most recent last).
 * Example: [68, 28, 0] → first log was 68s ago, last is "now".
 */
...
/** Format elapsed time — full: "Xd Yh Zm Ws", short: "Xd Yh Zm" */
...
// ─── Career start date — used to compute real uptime ──────────────────────
// Set this to when you started your first data engineering role
const CAREER_START = new Date('2024-01-05T09:00:00');
...
function UptimeTicker() {
  // Start with null — renders nothing on server, avoids hydration mismatch
  const [full,  setFull]  = useState<string | null>(null);
  const [short, setShort] = useState<string | null>(null);

  useEffect(() => {
    // Populate immediately on client mount, then tick every second
    setFull(formatUptime(CAREER_START, false));
    setShort(formatUptime(CAREER_START, true));
  ...
  // Render nothing until client has mounted — no server/client mismatch
...
export function CoreFundamentals() {
  ...
  // Build real log timestamps client-side only — avoids server/client mismatch.
  // null until mounted; LogLine renders an empty string for ts when null.
  const [dsaTimes, setDsaTimes] = useState<string[] | null>(null);
  const [sdTimes,  setSdTimes]  = useState<string[] | null>(null);

  useEffect(() => {
    setDsaTimes(buildLogTimes([68, 28, 0]));
    setSdTimes(buildLogTimes([1566, 576, 126]));
  }, []);
  ...
  return (
    <motion.div
      ...
      className="relative rounded-2xl overflow-hidden border border-primaryGlow/18"
    ...
    >
      ...
      <div className="relative flex items-center justify-between px-4 md:px-5 py-2.5 bg-black/30 border-b border-white/5">
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          {/* Label hidden on mobile — not enough room alongside uptime */}
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-white/5">
        ...
        <div className="p-6 md:p-8 flex flex-col gap-5 justify-center">
          ...
          <div>
            ...
            <p className="text-sm text-textSecondary leading-relaxed">
              Daily deliberate practice on the pillars that separate engineers who ship fast from those who ship right. Scalability starts at the algorithm.
            </p>
          </div>
        </div>

        {/* Right — process cards with real timestamps */}
        <div className="p-4 md:p-5 flex flex-col gap-3">
          <ProcessCard
            ...
            sub="Python · C++"
            logs={[
              { ts: dsaTimes?.[0] ?? '', level: 'ok',  text: 'Solved: sliding window O(n)' },
              { ts: dsaTimes?.[1] ?? '', level: 'run', text: 'Graph BFS — 3 test cases'    },
              { ts: dsaTimes?.[2] ?? '', level: 'ok',  text: 'All passed'                  },
            ]}
          />
          ...
          <ProcessCard
            ...
            logs={[
              { ts: sdTimes?.[0] ?? '', level: 'ok',  text: 'Designed: rate limiter'     },
              { ts: sdTimes?.[1] ?? '', level: 'run', text: 'Review: consistent hashing' },
              { ts: sdTimes?.[2] ?? '', level: 'ok',  text: 'Deployed to notes'          },
            ]}
          />
        </div>
      </div>
