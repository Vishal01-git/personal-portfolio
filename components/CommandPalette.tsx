...
import {
  Network, Database, Cpu, Terminal, User, Mail,
  Search, Clock, ArrowRight, Hash, X
} from 'lucide-react';
...
const COMMANDS = [
  ...
  { id: 'github',       label: 'GitHub',         sublabel: 'github.com/Vishal01-git',   href: 'https://github.com/Vishal01-git', icon: <ArrowRight className="w-4 h-4" />, category: 'Links' },
  { id: 'linkedin',     label: 'LinkedIn',       sublabel: '/in/vishalprajapati07',     href: 'https://linkedin.com/in/vishalprajapati07', icon: <ArrowRight className="w-4 h-4" />, category: 'Links' },
  { id: 'resume',       label: 'Download Resume',sublabel: 'PDF · latest version',      href: '/resume.pdf',    icon: <ArrowRight className="w-4 h-4" />, category: 'Links' },
];
...
export function CommandPalette() {
  ...
  const results = COMMANDS.filter(cmd =>
    fuzzyMatch(query, cmd.label) || fuzzyMatch(query, cmd.sublabel)
  );

  // If no query, show recents at top
  const recentCmds = !query
    ? recent.map(id => COMMANDS.find(c => c.id === id)).filter(Boolean) as typeof COMMANDS
    : [];
  const otherCmds = !query
  ...
  useEffect(() => {
    setSelected(s => Math.min(s, Math.max(0, allVisible.length - 1)));
  }, [allVisible.length]);
  ...
  const navigate = useCallback((cmd: typeof COMMANDS[0]) => {
    ...
    setOpen(false);
    setQuery('');
    if (cmd.href.startsWith('http') || cmd.href.startsWith('/resume')) {
  ...
  // Group visible by category (only when querying)
  const grouped = query
    ? Object.entries(
        results.reduce((acc, cmd) => {
  ...
  return (
    <>
      {/* Trigger hint in navbar — rendered by Navbar, but global shortcut is always active */}
      <AnimatePresence>
        {open && (
          <>
            {/* Backdrop */}
            ...
            >
              <div className="bg-surface border border-white/10 rounded-2xl shadow-elevation overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/8">
                  ...
                  />
                  {query && (
                    <button onClick={() => setQuery('')} className="text-textSecondary hover:text-textPrimary transition-colors">
                      <X className="w-3.5 h-3.5" />
                    </button>
                  )}
                  <kbd className="hidden sm:flex items-center gap-1 text-[10px] font-mono text-textTertiary bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">
                    esc
                  </kbd>
                </div>
                ...
                <div className="max-h-[340px] overflow-y-auto py-2">
                  {allVisible.length === 0 && (
                    <div className="px-4 py-8 text-center text-textSecondary font-mono text-sm">
                  ...
                  {query && grouped && grouped.map(([category, cmds]) => (
                    <div key={category}>
                      <div className="px-4 py-1.5 text-[10px] font-mono uppercase tracking-widest text-textTertiary">
                        {category}
                      </div>
                      {cmds.map(cmd => {
                        const idx = globalIdx++;
                  ...
                  {/* Non-query: recents then all */}
                  {!query && (
                    <>
                      {recentCmds.length > 0 && (
                        <div>
                          <div className="px-4 py-1.5 text-[10px] font-mono uppercase tracking-widest text-textTertiary flex items-center gap-1.5">
...
function CommandRow({
  cmd, isSelected, onHover, onClick
}: {
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors duration-100 ${
        isSelected ? 'bg-primaryGlow/10' : 'hover:bg-white/4'
      }`}
    >
      <div className={`shrink-0 transition-colors ${isSelected ? 'text-primaryGlow' : 'text-textSecondary'}`}>
        {cmd.icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-medium transition-colors ${isSelected ? 'text-white' : 'text-textPrimary/80'}`}>
          {cmd.label}
        </div>
...
export function CommandPaletteTrigger() {
  return (
    <button
      onClick={() => {
        // Dispatch synthetic keyboard event to open palette
    ...
    >
      ...
      <span>Search</span>
      <kbd className="flex items-center gap-0.5 text-[10px] text-textTertiary ml-1">
